...
local function DrawRow(x, y, label, value)
    -- Draw text centered within row (y is top of row, row extends downward by rowHeight)
    local textY = y - CFG.rowHeight / 2 - 2
    SetColor(COL.labelText)
    glText(label, x, textY, CFG.labelFontSize, "o")
    SetColor(COL.valueText)
    glText(value, x + windowW - CFG.padding * 2, textY, CFG.labelFontSize, "or")
...
function widget:DrawScreen()
    ...
    elseif stratWarning and osClock() < stratWarning.expireTime then
        ...
        statusText = "Click values to cycle options"
    end
...
function widget:GetConfigData()
    return {
        ...
        windowY = windowY,
        collapsed = collapsed,
        automationLevel = (TL and TL.GetAutomationLevel) and TL.GetAutomationLevel() or 0,
